function [BLA, p] = draft(ILC_Measurements)
    %
    ...
    w     = 2*real(ifft(W));
    y = squeeze(ILC_Measurements.yj(:,end,:)).';
    p     = polyfit(w,y,5);
    ...
    subplot(2,2,1); plot(freq,db(BLA.mean),freq,db(BLA.stdNL));
    subplot(2,2,3); plot(freq,angle(BLA.mean));
    x = ILC_Measurements.u_ref(1,:);
    subplot(2,2,[2 4]); plot(x,polyval(p,x));
    
end

function BLA = linearPart(ILC_Measurements)
    % ILC_Measurements.u_ref is M x N 
    ...
    % Select Excited frequencies (these are synced)
    ...
    % replicating the same period twice, FDT does not allow single period
    % measurements
